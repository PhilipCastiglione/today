...
class TaskTemplate < ApplicationRecord
  ...
  def repeats?
    repeat_days.present? && repeat_days > 0
  end
  ...
  def next_instance
    ...
    start_date = if most_recent_instance.nil?
      ...
      (most_recent_instance.completed_at || most_recent_instance.due_date) +
        (most_recent_instance.state == TaskInstance::STATES[:failed] ? 1 : repeat_days).days
    end
    ...
    task_instances.create(
      ...
      due_date: start_date + days_to_complete.days - 1,
      goal_type: goal_type,
